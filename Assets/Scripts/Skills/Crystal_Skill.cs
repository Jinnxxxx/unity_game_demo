...
public class Crystal_Skill : Skill
{
    ...
    public override void UseSkill()
    {
        ...
        else
        {
            ...
            // 检查是否克隆代替水晶（水晶不会爆炸，clone_skill取而代之）
    ...
    public void CreateCrystal()
    {
        ...
        // 设置水晶的持续时间, 是否可以爆炸, 是否可以移动到敌人, 移动速度, 最近的敌人, player引用
    ...
    private bool CanUseMultiCrystal()
    {
        if (canUseMultiStacks)
        {
            ...
            if (crystalLeft.Count > 0)
            {
                ...
                GameObject crystalToSpawn = crystalLeft[crystalLeft.Count - 1]; // 从列表中取出剩余水晶中的最后一个水晶
                ...
                newCrystal.GetComponent<Crystal_Skill_Controller>().
                    SetupCrystal(crystalDuration, canExplode, canMoveToEnemy, moveSpeed, FindClosestEnemy(newCrystal.transform), player); // 初始化水晶脚本
    ...
    {
        // 防止已经在Cooldown的时间内，不合理的调用预约的ResetAbility
        if (cooldownTimer > 0)
            return;
